  ...
  public class PowerData
  {
    ...
    public void Apply()
    {
      ...
      if (this.IsNew)
      {
        ...
        DatabaseAPI.Database.Power[this.Index] = (IPower) new Power();
      }
      if (!(!this.IsNew & this.Index < 0) && !DatabaseAPI.Database.Power[this.Index].NeverAutoUpdate)
      {
    ...
    public bool CheckDifference(out string message)
    {
      ...
      else
      {
        ...
        {
          for (int index = 0; index <= DatabaseAPI.Database.Power[this.Index].BoostsAllowed.Length - 1; ++index)
          {
