    ...
    public static class FileAssocation
    {
        ...
        static bool CheckAddFileOpenWithListItem(string extension)
        {
            ...
            using (var owlKey = GetFileOpenWithListKey(extension))
            {
                ...
                // already present
                if (itemNames.Any(name => string.Equals(MidsContext.AppName, (string)owlKey.GetValue(name)))) return false;
                // error condition-ish
                ...
                if (mruListValue != null && mruListValue.Contains(nextLetter))
                    return false;
                owlKey.SetValue(nextLetter.ToString(), MidsContext.AppName);
